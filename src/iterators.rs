...
pub struct Keys<'a, K, V> {
    ...
    //from the HashMap.
    pub buckets: &'a Vec<Bucket<K, V>>,
}

impl<'a, K, V> Keys<'a, K, V> {
    pub fn new(buckets: &'a Vec<Bucket<K, V>>) -> Self {
...
impl<'a, K, V> Iterator for Keys<'a, K, V> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            if let Bucket::Occupied((key, _)) = current_item {
                return Some(key);
            }
        }
    }
...
impl<'a, K, V> Iterator for Values<'a, K, V> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            if let Bucket::Occupied((_, value)) = current_item {
                return Some(value);
            }
        }
    }
...
impl<K, V> IntoIter<K, V> {
    pub fn new(buckets: Vec<Bucket<K, V>>) -> Self {
        IntoIter { buckets }
    }
}
...
impl<K, V> Iterator for IntoIter<K, V> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        while !self.buckets.is_empty() {
            ...
            if let Bucket::Occupied((key, value)) = next_item {
...
impl<'a, K, V> Iterator for Iter<'a, K, V> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            let current_item = self.buckets.get(self.iterator_position)?;
            self.iterator_position += 1;
            if let Bucket::Occupied((key, value)) = current_item {
                return Some((key, value));
            }
        }
    }
