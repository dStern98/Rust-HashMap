...
The HashMap begins with all of the elements set to `Bucket::Vacant`. When a (key,value) pair is inserted, the vector element is changed to `Bucket::Occupied(K,V)`,
containing the Key, Value pair in question. When a field is deleted, the deletion is lazy, so the enum in question is replaced with `Bucket::Deleted`. The HashMap ignores deleted fields for the purpose of search, but will allow a new insertion to replace a deleted field.
Deleted fields are cleaned up only when the array is resized. The trade-off is therefore faster deletion but more frequent array resizes. The HashMap vector is sparse, and could be optimized in terms of memory and iteration.
